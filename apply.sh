...
original_author="Dmitry466"
original_name="test_flask"
original_urlname="test-flask"
original_description="Awesome test_flask created by Dmitry466" 
...
rm -rf "${name}"
rm -rf tests
cp -R ${TEMPLATE_DIR}/test_flask "${name}"
...
then
    ...
    echo "then run 'test_flask' or 'python -m test_flask'"
else
    poetry install
    echo "Applied Flask template"
    echo "Ensure you activate your env with 'poetry shell'"
    echo "then run 'test_flask' or 'python -m test_flask' or 'poetry run test_flask'"
fi
