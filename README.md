...
- 🎯 Entry points to execute your program using `python -m <test_flask>` or `$ test_flask` with basic CLI argument parsing.
...
---
# test_flask Flask Application

Awesome test_flask created by Dmitry466
...
```bash
git clone https://github.com/Dmitry466/test-flask test_flask
cd test_flask
make install
```
...
```bash
test_flask create-db   # run once
test_flask populate-db  # run once (optional)
test_flask add-user -u admin -p 1234  # ads a user
test_flask run
```
